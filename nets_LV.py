...
class VQVAE(nn.Module):
    def __init__(self, d, n_channels, code_size, n_block = 3, n_res_block = 4, cond_channels = None, dropout_p = .5,
        ...
        down = [nn.Conv2d(d + (0 if cond_channels is None else cond_channels),
                          n_channels[0], kernel_size = 7,padding = 3 ), #padding = 3
    ...
    def forward(self, x, cond = None):
        ...
        alignment_loss =(z - e.detach()) ** 2
        diff1 = torch.mean(alignment_loss)
        diff2 = torch.mean((e - z.detach()) ** 2)
#         return x_tilde, diff1 + diff2
        return x_tilde, diff1+diff2, alignment_loss

    def decode_codebook(self, z, cond = None):
        # z = self.encode(x, cond)
        e, e_st, _ = self.codebook(z)
        x_tilde = self.decode(e_st,cond)
        return x_tilde
    ...
    def loss(self, x, cond = None, reduction = 'mean'):
        x_tilde,diff,alignment_loss = self(x,cond)
        x = x.unsqueeze(1)
        ...
        return OrderedDict(loss=loss, recon_loss=recon_loss, x_tilde= x_tilde, reg_loss = diff, alignment_loss = alignment_loss)
...
class VQLatentSNAIL(PixelSNAIL):
    ...
    def forward(self, x, cond = None):
        ...
        codes = self.retrieve_codes(x)
        return super(VQLatentSNAIL,self).forward(codes)
    ...
    def loss(self, x, cond = None, reduction = 'mean'):
        ...
        return OrderedDict(loss=nll, x_tilde = logits, code = code)

    def sample(self, n, img_size = (38,38), cond = None):
        device = next(self.parameters()).device
        samples = torch.zeros(n, *img_size).long().to(device)
        logit = torch.zeros(n,128, *img_size).long().to(device)
        with torch.no_grad():
        ...
        return samples,logit

    def generate_latent(self, x, cond = None):
        # Retrieve codes for images
        codes = self.retrieve_codes(x)
        logits = super(VQLatentSNAIL,self).forward(codes)
        probs = F.softmax(logits, dim=1)
        prob2, preds= torch.max(probs, dim=1)
        # pred2 = torch.multinomial(probs, 1).squeeze(-1)
        return logits,codes,prob2,preds
